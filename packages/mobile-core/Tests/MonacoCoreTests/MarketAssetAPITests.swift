...
final class MarketAssetAPITests: XCTestCase {
    ...
    func testGetMarketAssetChart_sendsEveryRangeTheBackendSupports() async throws {
        for range in AssetChartRange.allCases {
            ...
            let chart = try await makeClient().getMarketAssetChart(symbol: "AAPLc", range: range)
    ...
    func testGetMarketAssetChart_decodesADenseCandleSeries() async throws {
        ...
        let chart = try await makeClient().getMarketAssetChart(symbol: "AAPLc", range: .oneDay)
    ...
    func testGetMarketAssetChart_emptySeriesKeepsItsReason() async throws {
        ...
        let chart = try await makeClient().getMarketAssetChart(symbol: "SPCXc", range: .oneYear)
    ...
    func testGetMarketAsset_decodesTheStatsGridAndTheComparison() async throws {
        ...
        let detail = try await makeClient().getMarketAsset(symbol: "AAPLc")
        ...
        XCTAssertEqual(detail.stockVsToken?.premiumBps, 50)
        XCTAssertEqual(detail.stockVsToken?.token.source, .dexKyber)
        XCTAssertEqual(detail.stockVsToken?.mark.priceUsdcMicros, detail.priceUsdcMicros)
    }
    ...
    func testGetMarketAsset_missingComparisonIsNotAnError() async throws {
        ...
        let detail = try await makeClient().getMarketAsset(symbol: "SPCXc")
    ...
    func testListMarketAssets_decodesTheSessionEnvelope() async throws {
        respond("""
        {
          "assets":[{"symbol":"AAPLc","name":"Apple","tokenAddress":"0xb200000000000000000000c2e324d24d7eecd1fb","routable":true,"priceUsdcMicros":232050000,"change24h":"0.0245"}],
          "hasMore":false,
    ...
    func testGetMarketAsset_malformedBodyStillThrows() async {
        respond(#"{"symbol":"AAPLc"}"#) // no name, token address or liquidity

        do {
            _ = try await makeClient().getMarketAsset(symbol: "AAPLc")
