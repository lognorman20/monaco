...
final class CabalPictureTests: XCTestCase {
    ...
    /// The app ends the session on this exact error (against the token the request
    /// sent), so a 401 must not be folded into `.rejected` even with a body.
    func testRemoveCabalPicture_401_staysABareStatus() async {
        MockURLProtocol.requestHandler = { request in
            (Self.response(request, status: 401), Data(#"{"error":"invalid or expired access token"}"#.utf8))
        }

        do {
            _ = try await makeClient().removeCabalPicture(groupID: "g1")
            XCTFail("expected rejection")
        } catch {
    ...
    func testUploadCabalPicture_malformedSuccessBodyThrowsInsteadOfInventingAPicture() async {
        MockURLProtocol.requestHandler = { request in
            (Self.response(request, status: 200), Data(#"{"pictureUrl":"https://cdn.test/g1.jpg"}"#.utf8))
        }

        do {
            _ = try await makeClient().uploadCabalPicture(groupID: "g1", imageData: Data([0x00]), mimeType: "image/jpeg")
            XCTFail("a body without groupId must not decode")
        } catch {
