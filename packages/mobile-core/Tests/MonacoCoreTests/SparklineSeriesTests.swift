...
final class TopMoversTests: XCTestCase {
    private func asset(_ symbol: String, change: String?) -> MarketAssetDTO {
        MarketAssetDTO(
            symbol: symbol, name: symbol, tokenAddress: "0xb2", routable: true, priceUsdcMicros: 1_000_000,
            change24h: change, change24hBasis: .underlying, change24hBasisSymbol: symbol
        )
    }

    /// A move nobody labelled is not ranked: beside the token's price it would read as the
    /// token's move.
    func testAnUnlabelledMoveIsNotAMover() {
        let unlabelled = MarketAssetDTO(
            symbol: "U", name: "U", tokenAddress: "0xb2", routable: true, priceUsdcMicros: 1_000_000, change24h: "0.5"
        )
        XCTAssertEqual(TopMovers.rank([unlabelled, asset("C", change: "0.04")]).map(\.symbol), ["C"])
    }
