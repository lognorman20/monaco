...
public final class MonacoAPIClient: @unchecked Sendable {
    // lane: notifications — internal, not private, so `MonacoAPIClient+<Feature>.swift` extensions can build requests.
    let baseURL: URL
    ...
    // lane: notifications — internal for feature extensions.
    func send(
