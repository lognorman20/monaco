...
/// token publishes them, and six honest cells beat nine with three invented.
///
/// The Kyber spread is not here either. It is the *token's* number and every cell in
/// this grid is the *equity's*, per share, from Pyth; the spread belongs on the
/// stock-vs-token card next to the two prices it is measured between, which is where
...
public struct AssetStatsGrid: Equatable, Sendable {
    ...
    ///
    /// - Parameter currentUsdcMicros: the price the 52-week bar places. Pass the
    ///   equity's own price when there is one: the range is the equity's, and a
    ///   marker placed from the token's total-return mark would drift up the bar
    ///   with every reinvested dividend even if the share had not moved. It is only
    ///   ever a marker position and is never shown as a figure.
