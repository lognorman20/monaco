...
`/v1/assets/{symbol}/chart` and `/v1/assets/{symbol}/social` return (and the market
figures on a cabal's holdings), and where every number comes from. `docs/api.md` is not
on `main` yet; when the API reference comes back, this becomes its market section.
