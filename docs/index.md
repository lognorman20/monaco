...
Monaco is an iOS app where friends form a group, pool USDC, and buy tokenized US stocks on Base. This folder is the build backlog for that app. It is a how-to, not an architecture essay. Product rules live in [`docs/product.md`](product.md). How to clone and run lives in the root [`README.md`](../README.md).
...
Each milestone is one shippable slice. It names the paths it owns, the tickets that land it, the `just` commands that prove the code, and the manual steps you run on device, Dynamic, local Postgres, and explorer.

M0 is scaffold. Your five product steps are M1 through M5.

1. Auth with SMS and email and password for testing, member wallets, and group treasury wallets via Dynamic.
2. User deposits and treasury sweeps.
3. Buy and sell tokenized stocks via Kyber.
...
| **Issue title** | `Mx-Ty: <ticket title>` — e.g. `M1-T4a: Verify Dynamic access token` |
...
2. **Test layers** — unit vs integration vs Swift; which fakes each layer uses (fake Dynamic, fake Kyber, local Postgres, etc.).
...
6. **Manual verification** — human-only steps: Dynamic dashboard, mainnet USDC, slim sim UX, explorer links. Does not repeat row counts or JSON shapes already in the test map.
...
M0 scaffold
  then M1 auth and wallets
    then M2 deposits and sweeps
      then M3 Kyber buy and sell
        then M4 domain logic
          then M5 mobile product UI
```

M1 may create a thin group so you can attach a treasury. M4 replaces that stub with join policy, voter set, threshold, and expiry. M3 may expose a temporary stub buy so you can hit Kyber before votes exist. M4 deletes that stub when a passed vote is the only execute path.

M1 login screens may show Base addresses so you can prove Dynamic wiring. M5 moves those addresses to Settings, Advanced and keeps the main flow free of wallet copy.
...
`packages/domain` is a Go module the API and its tests import. Do not share it into Swift. Mobile talks to `apps/backend` over HTTP. Swift never calls B20, Kyber, Pyth Hermes, or Base RPC for product flows. Dynamic Swift handles auth and member wallets only.
...
App names are `backend` and `mobile`. Wire them in M0. Each later milestone extends what `just test backend` or `just test mobile` runs internally. Do not add other top-level recipes such as `just db`, `just check`, `just bootstrap`, `just test-deposits`, `smoke-*`, or `ios-build`. `just install` is the clone setup script; `just encrypt`, `just decrypt`, and `just show-env` wrap dotenvx on `.env.local`; `just relayer balance` prints fee payer pubkey and mainnet ETH (no private key). Do not add `just build` or `just test` umbrellas.
...
- `just run mobile` builds and launches only the app on that sim, with Dynamic env injected. Start the API separately or use `just run`.
...
- Failed Kyber `/execute` after a passed vote.
- Creator leave and group dissolve.

## Product and ops

- [Product and architecture](product.md)
- [Sweep USDC out of Dynamic wallets](ops-sweep-wallets.md) (`./scripts/sweep-wallets.sh`)

## Milestone files

- [M0. Scaffold the monorepo](milestones/m0-scaffold.md)
- [M1. Auth and Dynamic wallets](milestones/m1-auth-wallets.md)
- [M2. Deposits and sweeps](milestones/m2-deposits.md)
- [M3. Kyber buy and sell](milestones/m3-jupiter.md)
