...
The app's xcconfig includes a generated `Dynamic.local.xcconfig`, so each round runs
`scripts/ensure-ios-dynamic-config.sh generate` first. That needs `DYNAMIC_ENVIRONMENT_ID` in
`.env.local`. Without it the round records `ios-config: fail` and skips the app steps rather
than building against a missing file; the backend and host tests still run.
