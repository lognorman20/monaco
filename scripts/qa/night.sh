...
while (( round < rounds )); do
  ...
  if (( ! skip_ui )); then
    if ! run_step "$round" ios-config 120 "$root/scripts/ensure-ios-privy-config.sh" "$ios_config"; then
      ...
      # The usual cause is PRIVY_APP_ID or PRIVY_APP_CLIENT_ID missing from .env.local; the step log says so.
      record "$round" app-build skipped 0 "no Privy config; see r${round}-ios-config.log"
    elif ! resolve_sim; then
