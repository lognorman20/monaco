...
// countingRPC wraps a wallets client and counts the treasury balance reads, which
// are live Base RPC calls in production.
...
// countingMarks wraps a marks client and counts the mark lookups behind it.
//
// Every Chainlink round is a chain read, so "one lookup per request however many
// cabals hold the stock" is a claim about round trips and is counted as one.
...
func (c *countingMarks) MarkedPot(ctx context.Context, treasury marks.TreasuryRef, holdings []marks.CostBasis) (marks.NavInput, error) {
	c.mu.Lock()
	c.calls++
	c.mu.Unlock()
	return c.Client.MarkedPot(ctx, treasury, holdings)
}

func (c *countingMarks) count() int {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.calls
}
