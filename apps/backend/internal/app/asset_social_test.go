...
func TestAssetHoldingRow_valuesUnitsAndReturn(t *testing.T) {
	...
	// 12 AAPLc at $232.05, bought for $2,600.00.
	build, err := assetHoldingRow("Weekend investors", "g1", marks.MarkedHolding{
		Symbol:    "AAPLc",
	...
	proposals := []postgres.SymbolProposalRow{
		{
			ProposalRow: postgres.ProposalRow{
				ID: "p-old", GroupID: "g1", Symbol: "AAPLc",
		...
		{
			ProposalRow: postgres.ProposalRow{
				ID: "p-new", GroupID: "g1", Symbol: "AAPLc",
...
func TestAssetActivityFeed_sellReportsProceedsNotTokenAtomics(t *testing.T) {
	...
	at := time.Date(2026, 9, 22, 14, 0, 0, 0, time.UTC)
	// The cabal sold 12 AAPLc for $2,784.60. transactions.amount holds the input
	...
	const soldAtomics = 12 * b20.TokenAtomicScale
...
func TestAssetActivityFeed_respectsLimitAndUTC(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		rows = append(rows, postgres.SymbolProposalRow{
			ProposalRow: postgres.ProposalRow{
				ID: string(rune('a' + i)), GroupID: "g1", Symbol: "AAPLc",
