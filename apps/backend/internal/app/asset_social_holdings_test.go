...
// failingTreasuryRPC fails every treasury balance read, standing in for a Solana RPC
// that is down.
...
func (f failingTreasuryRPC) TreasuryUSDCBalance(context.Context, string) (int64, error) {
	return 0, errors.New("solana rpc unavailable")
}
...
func TestAssetSocialHoldings_costIsFlatInCabalCount(t *testing.T) {
	...
	t.Logf("statements=%d for both 1 and 12 cabals, rpc=0:\n%s", len(manySQL), formatStatementCounts(manySQL))
}

// The card's numbers are the point of the card, so the batched read has to produce
// the same dollars the per-cabal read did: 12 AAPLx bought for $2,600, marked at
...
func TestAssetSocialHoldings_priceOutageShowsHoldingsAtCost(t *testing.T) {
	...
	pyth.RegisterMarkedPotError(fx.h.Pyth, pyth.TreasuryRef{}, errors.New("hermes unavailable"))
...
// The treasury balance is a live Solana read and this card never needed it: the units,
...
func TestAssetSocialHoldings_survivesATreasuryRPCOutage(t *testing.T) {
	...
	fx.home = NewHomeService(fx.store, failingTreasuryRPC{Client: fx.h.Privy}, fx.h.Pyth, nil, fx.h.Symbols)
...
func TestAssetSocialHoldings_unheldSymbolStopsAfterTheLedgerRead(t *testing.T) {
	...
	holdings, unvalued := fx.home.assetHoldings(context.Background(), fx.userID, fx.groupIDs, "TSLAx")
