...
func tokenAtomicsToDecimalUnits(atomics int64) (domain.ShareUnits, error) {
	...
	r := new(big.Rat).SetFrac(big.NewInt(atomics), big.NewInt(100_000_000))
...
func costBasisMarkPerUnitMicros(totalUSDCMicros, tokenAtomics int64) (int64, error) {
	...
	mark, err := domain.MulDivFloor(totalUSDCMicros, 100_000_000, tokenAtomics)
	if err != nil {
...
func costBasisForHoldings(
	...
	for _, holding := range holdings {
		...
		if symbols != nil {
			var multOk bool
			mult, multOk = symbols.ResolveUiMultiplier(ctx, holding.Mint, kind)
			if !multOk {
				mult = nil
			}
			...
			mult, _ = pyth.EffectiveUiMultiplier(nil, string(kind))
		}
...
func potRowsFromPythInput(ctx context.Context, symbols *SymbolResolver, input pyth.NavInput) ([]GroupViewPotRow, error) {
	...
	for _, holding := range input.Holdings {
		...
		if holding.Kind == string(xstocks.AssetKindPreIPO) && symbols != nil {
