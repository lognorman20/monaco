...
type GroupViewPotRow struct {
	...
	AfterHours  *bool
	TokenAmount string
	// ValueUsdcMicros and DollarPnLUsdcMicros are the figures ValueUsd and
	// DollarPnL were formatted from. Callers that do arithmetic on one position
	// (the viewer's slice of it, a total across cabals) should not have to parse
	// their way back out of "1248.50". Display only: pot NAV is computed from the
	// marks, never from these.
