...
// LookupBySymbol returns the row whose ticker is symbol, from xStocks or a
// supplemental source. xStocks answers from its catalogue index. A supplemental
// ticker's case can be the only thing telling two issuers apart, so an exact match
// wins over one that differs only in case.
//
// An xStocks failure does not hide a supplemental row: the error is returned only
// when no source has the symbol.
func (c *Composite) LookupBySymbol(ctx context.Context, symbol string) (xstocks.CatalogAsset, bool, error) {
	symbol = strings.TrimSpace(symbol)
	if symbol == "" {
		return xstocks.CatalogAsset{}, false, nil
	}
	var xstocksErr error
	if c.xstocks != nil {
		asset, ok, err := c.xstocks.LookupBySymbol(ctx, symbol)
		if err == nil && ok {
			return c.enrichAsset(ctx, asset.Normalize()), true, nil
		}
		xstocksErr = err
	}
	var folded *xstocks.CatalogAsset
	for _, asset := range c.supplementalRows(ctx) {
		ticker := strings.TrimSpace(asset.Symbol)
		if ticker == symbol {
			return asset, true, nil
		}
		if folded == nil && strings.EqualFold(ticker, symbol) {
			match := asset
			folded = &match
		}
	}
	...
	return xstocks.CatalogAsset{}, false, xstocksErr
}
