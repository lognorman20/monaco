...
func TestCompositeLookupBySymbol_findsXStockAndSupplementalRows(t *testing.T) {
	ctx := context.Background()
	xs := xstocks.NewFakeCatalogSearcher()
	xstocks.RegisterCatalogAsset(xs, xStockAAPL())
	c := NewComposite(xs, NewFakeSource(tesseraSpaceX()), nil)

	aapl, found, err := c.LookupBySymbol(ctx, "aaplx")
	if err != nil || !found || aapl.SolanaMint != aaplXMint {
		t.Fatalf("LookupBySymbol(aaplx) = %+v, %v, %v; want the AAPLx row", aapl, found, err)
	}
	spacex, found, err := c.LookupBySymbol(ctx, "tSpaceX")
	if err != nil || !found || spacex.Kind != xstocks.AssetKindPreIPO || spacex.Decimals != 9 {
		t.Fatalf("LookupBySymbol(tSpaceX) = %+v, %v, %v; want the Tessera row", spacex, found, err)
	}
	if _, found, err := c.LookupBySymbol(ctx, "NOPEx"); err != nil || found {
		t.Fatalf("LookupBySymbol(NOPEx) = found %v, err %v; want a clean miss", found, err)
	}
}

func TestCompositeLookupBySymbol_exactCaseWinsAmongSupplementalTickers(t *testing.T) {
	ctx := context.Background()
	other := tesseraSpaceX()
	other.Symbol = "TSPACEX"
	other.SolanaMint = "OtherIssuerMint1111111111111111111111111111"
	c := NewComposite(xstocks.NewFakeCatalogSearcher(), NewFakeSource(other, tesseraSpaceX()), nil)

	asset, found, err := c.LookupBySymbol(ctx, "tSpaceX")
	if err != nil || !found || asset.SolanaMint != tSpaceXMint {
		t.Fatalf("LookupBySymbol(tSpaceX) = %+v, %v, %v; want the exact-case row", asset, found, err)
	}
}

func TestCompositeLookupBySymbol_xStocksOutageStillFindsSupplementalRows(t *testing.T) {
	ctx := context.Background()
	xs := xstocks.NewFakeCatalogSearcher()
	outage := errors.New("xstocks catalogue unavailable")
	xstocks.RegisterCatalogSearchError(xs, outage)
	c := NewComposite(xs, NewFakeSource(tesseraSpaceX()), nil)

	asset, found, err := c.LookupBySymbol(ctx, "tSpaceX")
	if err != nil || !found || asset.SolanaMint != tSpaceXMint {
		t.Fatalf("LookupBySymbol(tSpaceX) = %+v, %v, %v; want the Tessera row despite the outage", asset, found, err)
	}
	if _, found, err := c.LookupBySymbol(ctx, "AAPLx"); !errors.Is(err, outage) || found {
		t.Fatalf("LookupBySymbol(AAPLx) = found %v, err %v; want the xStocks error when no source has it", found, err)
	}
}
