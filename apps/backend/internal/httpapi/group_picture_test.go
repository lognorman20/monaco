...
func newPictureFixtureWithLimiter(t *testing.T, limiter *ratelimit.Limiter) *pictureFixture {
	...
	service := app.NewGroupPictureService(multi.Store, multi.Privy, fakeStorage)
	if limiter != nil {
...
func TestUploadGroupPicture_notConfiguredWithoutStorage(t *testing.T) {
	multi := newMultiUserApp(t)
	handlers := &GroupPictureHandlers{Pictures: app.NewGroupPictureService(multi.Store, multi.Privy, nil)}
...
func TestGroupView_carriesPictureUrlAndIsCreator(t *testing.T) {
	...
	ownerView := f.groupView(t, owner, c.ID)
	...
	if memberView := f.groupView(t, member, c.ID); memberView.IsCreator {
	...
	for _, viewer := range []clubMember{owner, member} {
		view := f.groupView(t, viewer, c.ID)
	...
	if after := f.groupView(t, member, c.ID); after.PictureURL != nil {
		t.Fatalf("pictureUrl = %v after removal, want null", *after.PictureURL)
	}
}

func (f *pictureFixture) groupView(t *testing.T, member clubMember, groupID string) groupViewResponse {
	t.Helper()
	rec := f.call(t, f.Groups.GetGroupViewHandler, http.MethodGet, "/v1/groups/"+groupID+"/view", member.Token, "", "id", groupID)
