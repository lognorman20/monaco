...
func lookupCatalogAssetBySymbol(ctx context.Context, catalog xstocks.CatalogSearcher, symbol string) (xstocks.CatalogAsset, bool) {
	...
	// A ticker resolves through the catalogue's symbol index. Search is only for a
	// name or an alias ("T-SpaceX") the index does not key on.
	if asset, found, err := catalog.LookupBySymbol(ctx, symbol); err == nil && found {
