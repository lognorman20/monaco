...
func (h assetSocialHarness) get(t *testing.T, token privy.AccessToken, symbol string) (*httptest.ResponseRecorder, assetSocialResponse) {
	t.Helper()
...
func (h assetSocialHarness) createGroup(t *testing.T, token privy.AccessToken, name string) string {
	t.Helper()
...
func TestGET_assetSocial_showsOwnCabalsOpenVoteAndActivity(t *testing.T) {
	...
	session, token := seedAuthenticatedUser(t, h.iso, h.auth, h.privy, "social-member", "Social Member")
	groupID := h.createGroup(t, token, "Weekend investors")
	proposalID := h.seedSymbolProposal(t, groupID, session.UserID, "AAPLx")

	rec, payload := h.get(t, token, "AAPLx")
	if rec.Code != http.StatusOK {
...
func TestGET_assetSocial_neverLeaksAnotherCabalsVote(t *testing.T) {
	...
	h := newAssetSocialHarness(t)
	insiderSession, insiderToken := seedAuthenticatedUser(t, h.iso, h.auth, h.privy, "social-insider", "Insider")
	insiderGroup := h.createGroup(t, insiderToken, "Private cabal")
	h.seedSymbolProposal(t, insiderGroup, insiderSession.UserID, "AAPLx")

	// An outsider with a cabal of their own, and no vote on this symbol anywhere.
	_, outsiderToken := seedAuthenticatedUser(t, h.iso, h.auth, h.privy, "social-outsider", "Outsider")
	h.createGroup(t, outsiderToken, "Other cabal")

	rec, payload := h.get(t, outsiderToken, "AAPLx")
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200; body = %s", rec.Code, rec.Body.String())
	}
...
func TestGET_assetSocial_noCabals_returnsEmptyLists(t *testing.T) {
	...
	h := newAssetSocialHarness(t)
	_, token := seedAuthenticatedUser(t, h.iso, h.auth, h.privy, "social-lonely", "Lonely")

	rec, _ := h.get(t, token, "AAPLx")
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200; body = %s", rec.Code, rec.Body.String())
	}
...
func TestGET_assetSocial_symbolIsValidatedServerSide(t *testing.T) {
	...
	h := newAssetSocialHarness(t)
	_, token := seedAuthenticatedUser(t, h.iso, h.auth, h.privy, "social-validate", "Validate")
...
func TestIsPlausibleTicker(t *testing.T) {
	...
	for _, ok := range []string{"AAPLx", "aaplx", "BRK.B", "TSLAx", "X", "SPY-1"} {
