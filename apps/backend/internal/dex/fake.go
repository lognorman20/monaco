...
func buyKey(tokenOut string, usdcIn *big.Int) string {
	return fmt.Sprintf("buy:%s:%s", tokenOut, usdcIn.String())
}

func sellKey(tokenIn string, amountIn *big.Int) string {
	return fmt.Sprintf("sell:%s:%s", tokenIn, amountIn.String())
}

func swapKey(q Quote) string {
	return fmt.Sprintf("swap:%s:%s:%s", q.TokenIn, q.TokenOut, q.AmountIn.String())
}
