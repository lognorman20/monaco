...
// GroupSymbolHolding is one cabal's net position in one token, with the cost basis
// behind the units it still holds.
type GroupSymbolHolding struct {
	GroupID string
	// Token is the B20 token's contract address on Base.
	Token string
...
// ListTreasuryAddressesByGroupIDs returns group id to treasury EVM address. A group
...
func (s *Store) ListTreasuryAddressesByGroupIDs(ctx context.Context, groupIDs []string) (map[string]string, error) {
	...
	const selectSQL = `SELECT group_id, address FROM treasuries WHERE group_id = ANY($1::uuid[])`
...
// ListShareBaseForGroups is the set form of SumShareUnitsByGroup: outstanding
// pot-backed share units per group, ghost positions excluded, in one query.
//
// It is deliberately the same base every other screen divides by. Home, the group
// view and the redeem paths all place a member on SumShareUnitsByGroup alone, so a
// slice computed here against a wider base would put this card's "yours" a few
// percent below the same member's figure on Home, for the same money, on the same
// screenful. Whatever the right base for an in-flight redemption turns out to be, it
// has to change in one place for every caller at once.
//
// A group with no positions answers zero rather than being left out: to a caller
// dividing by this, a missing key and a zero base are different bugs.
func (s *Store) ListShareBaseForGroups(ctx context.Context, groupIDs []string) (map[string]int64, error) {
	...
	const selectSQL = `
SELECT want.id, COALESCE(pos.share_units, 0)
...
// ListGroupHoldings returns every group's remaining units per token, with the
// fill-derived cost basis behind them, in one query.
//
// It replaces, for the whole set of groups, what used to cost one
// ListNetTokenHoldingsByGroup plus one GetFillDerivedCostBasisByOutputToken per
...
// cost it paid.
//
// It returns every token rather than taking a symbol, because a ticker is not a
// database fact: only the B20 catalog can say which address is "AAPLc", and the
// caller holds that resolver. Filtering here would mean either a second round trip to
...
func (s *Store) ListGroupHoldings(ctx context.Context, groupIDs []string) ([]GroupSymbolHolding, error) {
	...
	const selectSQL = `
WITH buys AS (
  SELECT group_id, output_token AS token,
  ...
  GROUP BY group_id, output_token
),
sells AS (
  SELECT group_id, input_token AS token, COALESCE(SUM(amount), 0) AS tokens
  FROM transactions
  WHERE group_id = ANY($1::uuid[]) AND action = 'sell' AND status = 'confirmed'
  GROUP BY group_id, input_token
)
SELECT buys.group_id, buys.token, buys.usdc, buys.tokens, COALESCE(sells.tokens, 0)
FROM buys
LEFT JOIN sells ON sells.group_id = buys.group_id AND sells.token = buys.token
WHERE buys.tokens - COALESCE(sells.tokens, 0) > 0
ORDER BY buys.group_id, buys.token`
	...
	for rows.Next() {
		var groupID, token string
		var buyUSDC, buyTokens, sellTokens int64
		if err := rows.Scan(&groupID, &token, &buyUSDC, &buyTokens, &sellTokens); err != nil {
		...
		holding := GroupSymbolHolding{GroupID: groupID, Token: token, Units: remaining}
		remainingBasis, err := domain.MulDivFloor(buyUSDC, remaining, buyTokens)
		if err != nil {
			return nil, fmt.Errorf("remaining cost basis for group %s token %s: %w", groupID, token, err)
		}
		if remainingBasis > 0 {
