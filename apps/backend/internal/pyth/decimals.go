...
func EffectiveUiMultiplier(mult *big.Rat, kind xstocks.AssetKind) (*big.Rat, bool) {
	if mult != nil {
		if mult.Sign() <= 0 {
			return nil, false
		}
...
func TokenAtomicsToDecimalUnits(atomics int64, decimals int) (domain.ShareUnits, error) {
	return TokenAtomicsToScaledDecimalUnits(atomics, decimals, big.NewRat(1, 1), xstocks.AssetKindStock)
}

// TokenAtomicsToScaledDecimalUnits formats raw atomics as scaled decimal units (raw × multiplier / 10^decimals).
func TokenAtomicsToScaledDecimalUnits(atomics int64, decimals int, uiMultiplier *big.Rat, kind xstocks.AssetKind) (domain.ShareUnits, error) {
...
func CostBasisMarkPerUnitMicros(totalUSDCMicros, tokenAtomics int64, decimals int, uiMultiplier *big.Rat, kind xstocks.AssetKind) (int64, error) {
	decimals = NormalizeTokenDecimals(decimals)
...
func HoldingValueUSDCMicros(rawAtomics, markUsdcMicros int64, decimals int, uiMultiplier *big.Rat, kind xstocks.AssetKind) (int64, error) {
	if rawAtomics < 0 {
...
func RawAtomicsForOneScaledUnit(decimals int, uiMultiplier *big.Rat) (int64, error) {
	decimals = NormalizeTokenDecimals(decimals)
	mult, ok := EffectiveUiMultiplier(uiMultiplier, xstocks.AssetKindStock)
	if !ok || mult.Sign() <= 0 {
		return 0, fmt.Errorf("ui multiplier unresolved")
	}
	scale := new(big.Int).SetInt64(jupiter.AtomicScale(decimals))
