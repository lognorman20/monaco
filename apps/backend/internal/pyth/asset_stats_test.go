...
func TestSessionStats_carriesTheBasisOfTheSeriesItFolded(t *testing.T) {
	...
	// These cells are NASDAQ dollars per share. The detail response puts them next
	// to the token's total-return mark, per token, so a hero price above the day's
	// high is two instruments rather than a bug — but only if the grid says which
