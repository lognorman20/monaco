...
func TestCostBasisMarkPerUnitMicros_eightDecimals(t *testing.T) {
	...
	mark, err := CostBasisMarkPerUnitMicros(totalUSDC, tokenAtomics, jupiter.XStockDecimals, big.NewRat(1, 1), xstocks.AssetKindStock)
	if err != nil {
...
func TestCostBasisMark_nineDecimals(t *testing.T) {
	...
	mark, err := CostBasisMarkPerUnitMicros(totalUSDC, tokenAtomics, 9, big.NewRat(1, 1), xstocks.AssetKindPreIPO)
	if err != nil {
		t.Fatalf("CostBasisMarkPerUnitMicros: %v", err)
	}
	if mark != 500_000_000 {
...
func TestMarkedPot_spacexPreStocks_multiplier5_valuesFiveTimesRaw(t *testing.T) {
	...
	oldValue, err := HoldingValueUSDCMicros(raw, mark, decimals, big.NewRat(1, 1), xstocks.AssetKindPreIPO)
	if err != nil {
		t.Fatal(err)
	}
	scaledValue, err := HoldingValueUSDCMicros(raw, mark, decimals, mult, xstocks.AssetKindPreIPO)
	if err != nil {
		t.Fatal(err)
	}
...
func TestProperty_potValueInvariantToMultiplier(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		...
		left, err := HoldingValueUSDCMicros(raw, mark, 9, mult, xstocks.AssetKindPreIPO)
		if err != nil {
			t.Fatalf("left: %v", err)
		}
		right, err := HoldingValueUSDCMicros(raw*k, mark, 9, big.NewRat(multInt/k, 1), xstocks.AssetKindPreIPO)
		if err != nil {
