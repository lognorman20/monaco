...
// ParsePrivateKey normalizes SOLANA_RELAYER_PRIVATE_KEY-style values to base58 and validates length.
//
// Accepted inputs:
...
func ParsePrivateKey(raw string) (string, error) {
	...
	if err != nil {
		if strings.ContainsRune(raw, '[') || strings.ContainsRune(raw, ']') {
			return "", fmt.Errorf("invalid Solana private key: %v (SOLANA_RELAYER_PRIVATE_KEY must be base58, not a JSON [1,2,...] array; re-export from solana-keygen or Phantom as base58)", err)
		}
		return "", fmt.Errorf("invalid Solana private key: %v (SOLANA_RELAYER_PRIVATE_KEY must be a base58-encoded 32- or 64-byte secret key)", err)
	}
...
func normalizePrivateKeyInput(raw string) (string, error) {
	...
	if strings.HasPrefix(raw, "[") {
		...
		if err != nil {
			return "", fmt.Errorf("SOLANA_RELAYER_PRIVATE_KEY looks like JSON array but failed to parse: %v (use base58 secret key string instead of [1,2,...])", err)
		}
		return encodeBase58(bytes), nil
	}

	if strings.ContainsRune(raw, '[') || strings.ContainsRune(raw, ']') {
		return "", fmt.Errorf("SOLANA_RELAYER_PRIVATE_KEY contains brackets; set a base58-encoded Solana secret key, not a JSON byte array")
	}
