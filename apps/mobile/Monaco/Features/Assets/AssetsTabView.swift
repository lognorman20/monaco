...
struct AssetsTabView: View {
    ...
    /// `model` is the seam the sample harness uses: some states (rows on screen plus a
    /// failed refresh) are a sequence of two responses, not one canned answer, so the
    /// harness drives the model into them before the view appears.
    init(auth: DynamicAuthService, sources: StocksFlowSources = .live, model: StocksTabModel? = nil) {
        self.auth = auth
        self.sources = sources
        _model = State(initialValue: model ?? StocksTabModel(dataSource: sources.tab ?? LiveStocksTabDataSource(auth: auth)))
    }
    ...
    private var popularSection: some View {
        section("Popular", identifier: "assets-popular") {
            ...
            Text(AssetsTabView.figuresFootnote)
                ...
                .accessibilityIdentifier("assets-figures-footnote")
        }
    }
    ...
    private func assetList(_ rows: [MarketRowData], identifierPrefix: String) -> some View {
        MonacoGroupedList {
            ForEach(rows) { row in
                ...
                // A token that is not listed for buying opens nothing: the detail's
                // whole point is Buy. Held and up-for-vote rows are still listed.
                .disabled(!row.asset.canBuy)
                .opacity(row.asset.canBuy ? 1 : 0.6)
    ...
    /// Placeholder rows while a list has nothing to show yet.
