...
struct AssetChartCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: MonacoTheme.Space.sm) {
            ...
            if let caption = model.series?.basisCaption {
                // The curve is usually the underlying equity's, per share; the price
                // above it is the B20 token's Chainlink mark, per token, which carries
                // the token's multiplier. Say so, or a curve ending below the hero
                // price reads as a bug instead of as two instruments.
                Text(caption)
    ...
    private func curve(_ series: AssetChartSeries) -> some View {
        MonacoScrubChart(
            ...
            summary: model.chartAccessibilitySummary,
            ...
            describePoint: { model.describePoint(at: $0) },
    ...
    private func reload() {
        Task { _ = await model.loadChart(range: model.range) }
    }
