...
struct AssetDetailHero: View {
    ...
    /// Replaces the name line while the price shown is not the token's: a finger on an
    /// underlying curve shows the share's price, "AAPL on its home exchange".
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: MonacoTheme.Space.s) {
            Text(priceCaption ?? displayName)
                .font(MonacoTheme.Typo.caption)
                .foregroundStyle(priceCaption == nil ? MonacoTheme.muted : MonacoTheme.ink)
