...
/// The best-effort Privy revoke a sign-out leaves running, and the bounded wait a new
/// sign-in does on it.
///
/// Sign-out is local-first: the session is cleared before Privy is told, so the login screen
/// comes back at once even offline. `user.logout()` is an unbounded network call — it is
...
/// disabled for as long as Privy takes, with no cancel and no way back but force-quitting;
...
/// told to end is still the open one before calling Privy.
@MainActor
