...
///
/// Only the current token and the one it replaced are kept. Dynamic rotates the access token
/// (`minAuthTokenChanges`), and the only tokens an in-flight reply can legitimately name are
/// the one it was sent with and the one that just superseded it. Holding every token a
/// multi-day session ever used would keep dozens of live credentials in the heap for no
/// added protection.
///
/// A plain value type, so the guard can be tested without standing up Dynamic.
