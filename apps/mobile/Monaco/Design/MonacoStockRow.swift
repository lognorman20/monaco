...
extension PnLTone {
    ...
    ///
    /// On Base both are the underlying's, from one Pyth series, and the line takes
    /// the pill's tone. When the row is told they are different instruments (a token
    /// line beside the share's move) it tints the line from the line, so the colour
    /// is about the picture on screen rather than a number measured somewhere else.
...
/// The filled capsule on the right of a market row: the share's day move, "+1.24%",
/// or "+$2.84" per share once tapped. Tapping toggles every pill in the app.
///
/// It only ever shows a move the backend labelled as the underlying's
/// (`StockDayMove`): beside the token's price an unlabelled percentage reads as the
/// token's move. VoiceOver hears it as "AAPL day move".
struct DayChangePill: View {
    let dayMove: StockDayMove?
    /// The underlying's latest price the dollar face is measured on; nil keeps the
    /// pill on percent. See `MarketRowData.dayMoveReferencePriceUsdcMicros`.
    let referencePriceUsdcMicros: Int64?
    var style: MoneyStyle = .caption

    private var change24h: String? { dayMove?.ratio }
    private var priceUsdcMicros: Int64? { referencePriceUsdcMicros }
    ...
    var body: some View {
        Text(label)
            ...
            .padding(-DayChangePill.tapTargetPadding)
            .accessibilityLabel(dayMove?.caption ?? "Day move")
        ...
        if layout.isStacked {
            VStack(alignment: .leading, spacing: MonacoTheme.Space.s) {
                ...
                HStack(spacing: MonacoTheme.Space.s) {
                    price
                    DayChangePill(dayMove: row.dayMove, referencePriceUsdcMicros: row.dayMoveReferencePriceUsdcMicros)
                }
            }
        } else {
            HStack(spacing: MonacoTheme.Space.sm) {
                ...
                if let spark = row.spark {
                    Sparkline(
                        series: spark,
                        tone: PnLTone(sparkTint: row.sparkTint, change24h: row.dayMove?.ratio)
                    )
