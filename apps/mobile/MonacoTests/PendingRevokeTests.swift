...
/// Signing out hands Dynamic's `sdk.auth.logout()` to a background revoke. A new sign-in waits for
/// it so a late revoke cannot tear down the session it is creating — but Dynamic's logout is
