...
struct DepositFailureToastTrackerTests {
    ...
    /// The backend writes a failed sweep as "failed" or "failed: <reason>"; both are failures, and
    /// neither pending nor confirmed ever reads as one.
    @Test func everyFailedSpellingIsAFailure() {
        #expect(FundStatus("failed") == .failed)
        #expect(FundStatus(" FAILED: submit_sweep ") == .failed)
        #expect(FundStatus("pending") == .pending)
        #expect(FundStatus("Confirmed") == .confirmed)
        #expect(FundStatus("failedish") == .other("failedish"))
    }

    @Test func aSweepThatFailsWithAReasonIsStillAnnounced() {
        DepositFailureToastTracker.resetForTesting()
        #expect(DepositFailureToastTracker.consumeNewFailures(from: [deposit("d1", "pending")]).isEmpty)
        let announced = DepositFailureToastTracker.consumeNewFailures(from: [deposit("d1", "failed: submit_sweep")])
        #expect(announced.map(\.id) == ["d1"])
    }
