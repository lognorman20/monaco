...
--
-- 000026 shipped join_waitlist(email, source, ip_hash, user_agent). Adding an argument
...
-- Re-runnable: this set first shipped as 000019-000022, and a database that ran those has
-- already renamed `name` to `twitter` (000028). Adding `name` back there would break 000028.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = current_schema() AND table_name = 'waitlist' AND column_name IN ('name', 'twitter')
  ) THEN
    ALTER TABLE waitlist ADD COLUMN name TEXT CHECK (name IS NULL OR length(name) <= 80);
  END IF;
END
$$;

-- On a re-run the five-argument function already exists with p_twitter (000028), and
-- CREATE OR REPLACE cannot rename a parameter.
