...
--
-- 000027 shipped an optional `name`; nothing meaningful depends on it yet, so this
...
-- Re-runnable: a database that ran this set as 000019-000022 already has `twitter`.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = current_schema() AND table_name = 'waitlist' AND column_name = 'name'
  ) THEN
